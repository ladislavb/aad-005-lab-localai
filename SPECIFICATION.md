...
- Pro CLI nástroj spusť nalezený executable jeho absolutní cestou s argumenty z `cli.versionArguments` v katalogu, popř. `--version` jako výchozí. Nepoužívej shell, PATH, `which`, `/bin/ps` ani jakékoli zjišťování běžících procesů. Nastav `LANG=C` a `LC_ALL=C`, zavři stdin a odděleně zachyť stdout a stderr, každý nejvýše do 4 KiB.
- Kontrola CLI použije synchronní `Process.run()` s timeoutem 3 sekundy implementovaným přes `DispatchSemaphore` nebo `Timer`. Před voláním `terminate()` ověř, že proces byl úspěšně spuštěn. Proces nesmí po skončení scanu zůstat běžet ani získat vstup uživatele. Za verzi považuj pouze jednoznačný token ve tvaru `v?MAJOR.MINOR.PATCH` s případným suffixem. Při selhání, nejednoznačném výstupu nebo chybějícím tokenu zobraz `Unavailable` s konkrétním důvodem; nezobrazuj celý výstup a verzi neodhaduj.
